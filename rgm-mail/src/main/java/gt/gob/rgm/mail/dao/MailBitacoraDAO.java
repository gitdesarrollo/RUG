...
import gt.gob.rgm.mail.exception.InfrastructureException;

public class MailBitacoraDAO {

	public int logMail(MailTO mailTO) {
...
//			                                          )
			...
			if(cs.getInt(9)==0){
				...
				//klanza excepcion
				throw new InfrastructureException(cs.getString(10));
			}
...
//             psTxResult       OUT  VARCHAR2
	...
	public MailTO getMailToSend(int id){
		ConexionDB conexionDB = new ConexionDB();
		try {
			ResultSet rs;
			conexionDB.prepareStatement("Select ID_MAIL,DESTINATARIO " +
		...
		} catch (SQLException | InfrastructureException e) {
			e.printStackTrace();
		} finally{
			conexionDB.destroy();
		}
