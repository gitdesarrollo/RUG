	...
	var imgDir = "/portal/imgs/popcalendar/"			// directory for images ... e.g. var imgDir="/img/"
    ...
    */
	...
	if (dom)
	{
		...
		document.write ("<div onclick='bShow=true' id='calendar'	style='z-index:+999;position:absolute;visibility:hidden;'><table	width="+((showWeekNumber==1)?250:220)+" style='font-family:arial;font-size:11px;border-width:1;border-style:solid;border-color:#a0a0a0;font-family:arial; font-size:11px}' bgcolor='#ffffff'><tr background='"+imgDir+"calback.jpg'><td background='"+imgDir+"calback.jpg'><table width='"+((showWeekNumber==1)?248:218)+"'><tr><td style='padding:2px;font-family:arial; font-size:11px;'><font color='BLACK'><B><span id='caption'></span></B></font></td><td align=right><a href='javascript:hideCalendar()' id=bclose tabindex=26><IMG SRC='"+imgDir+"close.gif' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#ffffff><span id='content'></span></td></tr>")
...
//---------------------------------------------TRADUCCION EN ESPAÑOL-----------------------------------------------------------------------------
	...
	function closeCalendar() {
		...
		//campoFecha.value=constructDate(dateSelected,monthSelected,yearSelected)
		_BuscaSelectOpcion(_anio,yearSelected);
		_BuscaSelectOpcion(_mes,monthSelected+1);
		llenaDias('anio','mes','dia');
		_BuscaSelectOpcion(_dia,dateSelected);
	}

	function _BuscaSelectOpcion(oControl, sValor){
		var iOpcion= 0;
...
//CAMBIA el siguiente for por este " for	(i=yearNow; i<=(yearNow+4); i++) { " si quieres que los años empizen en el actual en adelante		
			for	(i=yearNow; i>=parseInt(anioInicio); i--) {
				sName =	i;
				if (i==yearSelected){
					sName =	"<B>" +	sName +	"</B>"
				}
   ...
   function WeekNbr(n) {
	...
	function constructCalendar () {
		...
		if (showWeekNumber==1)
		{
			sHTML += "<td width=27><b>" + weekString + "</b></td><td width=1 rowspan=7 bgcolor='GREEN' style='padding:0px'><img src='"+imgDir+"divider.gif' width=1></td>"
		}

		for	(i=0; i<7; i++)	{
			sHTML += "<td width='27' align='right' style='color=WHITE; background-color:RED;'><B>"+ dayName[i]+"</B></td>"
		}
		sHTML +="</tr><tr>"
...
//------------------------------------TERMINAR LA FUNCION EN 0 PARA QUE INICIE EN EL DIA ACTUAL EN LA SIGUIENTE LINEA----------------------------------------------
			...
			{ sHTML += "<b><a tabindex="+(datePointer+27)+" "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected="+datePointer+";closeCalendar();'><font color=#ff0000>&nbsp;" + datePointer + "</font>&nbsp;</a></b>"}
			...
			{ sHTML += "<a tabindex="+(datePointer+27)+" "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected="+datePointer + ";closeCalendar();'>&nbsp;" + datePointer + "&nbsp;</a>" }
	...
	function popUpCalendar(a,m,d, ctl,	ctl2, format) {
		//campoFecha=ctl;
		//ctl=ctl2;
		_anio=a;
		_mes=m;
