package mx.gob.se.rug.factoraje.action;
...
public class FactorajeAction extends RugBaseAction{
...
private static final long serialVersionUID = 1L;
	...
	private List<DeudorTO> listaOtorgantes;
	...
	public String inicia(){		
		MyLogger.Logger.log(Level.INFO, "RUG-EspecialesAction>>iniciaInscripcionFormEspecias:::--entro a iniciaEspecial");
		...
		try {			
			...
			sessionMap.put(Constants.ID_TIPO_GARANTIA,new Integer(idTipoGarantia));
			
			setIdTipoGarantia(new Integer(idTipoGarantia));;
			cargarBienesEspeciales(getIdTipoGarantia());
			if(new Integer(idTipoGarantia)==4) {
			...
			setTextosFormulario(inscripcionService.getTextosFormularioByIdGarantia(getIdTipoGarantia()));
	...
	public String saveformulario() {
		...
		try {
			...
			if (idTramite != null && idTramite != 0) {
				...
				setListaOtorgantes(inscripcionService.getOtorgantesByTramite((Integer) sessionMap.get(Constants.ID_TRAMITE_NUEVO)));
				...
				for(DeudorTO vDeudor : getListaDeudores()) {
				...
				for(AcreedorTO vAcreedor : getListaAcreedores()) {
				...
				for(DeudorTO vDeudor : getListaOtorgantes()) {
					if(vDeudor.getTipoPersona().equalsIgnoreCase("PM")) {
						representante.append(vDeudor.getUbicacionInscrito());
						representante.append(". ");
					}
	...
	public List<DeudorTO> getListaOtorgantes() {
		return listaOtorgantes;
	}

	public void setListaOtorgantes(List<DeudorTO> listaOtorgantes) {
		this.listaOtorgantes = listaOtorgantes;
	}
